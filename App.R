           ...
           tabItem(tabName = "home",
                 ...
                 column(12,
                        tags$div(id= "data description", style= "color:	#221f72; font-size:14px; margin-left: 20px",
                              h3("Data Notes"),
                             #h5("1. Only includes Adult Med Surg, ICU, and ED."),
                              h5("1. Inaccurate census data on 5/17/2020 and 1/19/2021 due to an Epic Upgrade and outage, respectively."),
                              h5("2. COVID census data not available for 7/11/2021."),
                              h5("3. MSBI: data starts from its EPIC go-live date on 8/9/2020."),
                              h5("4. MSSN: ED vs. IP breakdown is not availble due to data limitation."),
                              h5("5. MSSN: Excludes PUI, PUM, and SUSC data due to data limitations."))),
                  ),
      ...
      tabItem(tabName = "sites",
              ...
              fluidRow(
                         ...
                         fluidRow(
                           box(width = 6, height = "100px", title = "Select Site:", solidHeader = F,
                               pickerInput("selectedSite",label= NULL, multiple= F,
                                           choices = sort( unique(covid_data$Site)),  selected = "MSH")),
                ...
                conditionalPanel(condition = "input.selectedSite== 'MSB'|input.selectedSite== 'MSBI'| input.selectedSite== 'MSH'| 
                  ...
                  column(11,
                         box( width= 12,
                             title = "Adult Med Surg: Total COVID-19 Patients Census", status = "primary",
                             solidHeader = TRUE, collapsible = TRUE, closable = TRUE,
...
server <- function(input, output, session) { 
  ...
  output$mshsName_DateShow <- renderText({
    paste0("Based on data from ", input$DateRange[1]," to ", input$DateRange[2])
        ...
        validate(need(nrow(site_data)>0, "Please provide a different start date"))
        ...
        validate(need(nrow(covid_pts_trend_site)>0, "Please provide a different start date"))
        ...
        output$site_plot2 <- renderPlot({
          ...
          validate(need(nrow(covid_pts_census_site)>0, "Please provide a different start date"))
        ...
        output$site_plot3 <- renderPlot({
          ...
          validate(need(nrow(covid_pts_census)>0, "Please provide a different start date"))
